...
const RevenueChart: React.FC<{ data: { month: string, amount: number }[] }> = ({ data }) => {
...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const revenueTrend = mockRevenues.length > 1 
      ? ((mockRevenues[mockRevenues.length - 1].amount - mockRevenues[0].amount) / mockRevenues[0].amount) * 100
      : 0
        ...
        >
          ...
          <div className="space-y-3">
            {mockTasks.slice(0, 3).map(task => (
              ...
              >
                <CheckSquare 
                  className={`mr-3 ${task.completed ? 'text-green-500' : 'text-blue-500'}`} 
                />
                <div>
